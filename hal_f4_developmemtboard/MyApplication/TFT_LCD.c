...
static uint32_t LCD_ReadID(void)
{
    ...
    buf[1] = LCD_Read_DATA() & 0x0000; // 只有低8位数据有效
...
static void LCD_Init(void)
{
    ...
    user_main_printf("The ID of TFT LCD is 0x%.6X\r\n", TFT_LCD.ID);
...
static void LCD_Disp_Direction()
{
    ...
    case 2:
        LCD_Write_CMD(0x36);
        LCD_Write_DATA((0 << 3) | (1 << 5) | (1 << 6));
        break;
    case 3:
        LCD_Write_CMD(0x36);
        LCD_Write_DATA((0 << 3) | (1 << 7) | (1 << 4) | (1 << 6));
        break;
    case 4:
        LCD_Write_CMD(0x36);
        LCD_Write_DATA((0 << 3) | (1 << 7) | (1 << 5) | (1 << 4));
        break;
    default:
        LCD_Write_CMD(0x36);
        LCD_Write_DATA(0 << 3);
